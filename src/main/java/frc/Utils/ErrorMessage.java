...
/**
 * @author Michael Andrzej Klaczynski Prints errors to the Drivers' Station
 *         and/or saves onto the RoboRIO. Messages on the RoboRIO are saved to
 *         /home/lvuser/errors/errorlog.txt by default, though you may create
 *         other log locations from this class, by constructor or method <br>
...
public class ErrorMessage
    ...
    /**
     * The default choice of whether or not to append a time Stamp to the message.
     */
    ...
    public ErrorMessage()
        {
            ...
            if (location.exists() == false)
                ...
                final String errorlogDirectory = this.errorlogLocation.substring(0, lastSlash);

                try
                    {
                    Runtime.getRuntime().exec("/bin/mkdir " + errorlogDirectory);
                    Runtime.getRuntime().exec("/bin/touch \"" + this.errorlogLocation + "\"");
    ...
    /**
     * Creates an ErrorMessage object and sends it to DS or RIO. * Prints to both
     * Drivers' Station and RoboRIO by default. <br>
     * Alternative constructor allows for choice to send to either device.<br>
     * Location defaults to /home/lvuser/errors/errorlog.txt Appends time stamp by
     * default
     *
     * @param PrintsTo
     ...
     *                                     whether or not to append a time stamp to
     *                                     each message by default.
     */
    ...
    /**
     * Creates an Error and decides to print to Drivers' Station and/or RoboRIO.
     * <br>
     ...
     * @param appendTimeStampByDefault
     *                                     is whether or not to append a time stamp
     *                                     to each message by default.
     */
     ...
     *                                     to each message by default.
     */
    public ErrorMessage(String errorlogLocation, boolean appendTimeStampByDefault)
        {
     ...
     *                                     to each message by default.
     */
    public ErrorMessage(String errorlogLocation, PrintsTo PrintsTo, boolean appendTimeStampByDefault)
        {
    ...
    /**
     * Appends date and time of the RIO and match time to the Error Message, spaced
     * with newlines.
     *
     * @param errorMessage
    ...
    public void clearErrorlog()
    {
        try
            {
            final PrintWriter log = new PrintWriter(new BufferedWriter(new FileWriter(errorlogLocation, false)));
            log.print("\n");
            log.close();
            } catch (final IOException e)
            {
    ...
    public void printError(String errorMessage, boolean attatchTimeStamp)
    {
        ...
        if (appendTimeStamp == true)
            appendedErrorMessage = appendErrorMessage(errorMessage);
        else appendedErrorMessage = errorMessage;
        ...
        // if the error must print to the errorlog on the rio.
        if (defaultPrintDevice == PrintsTo.roboRIO || defaultPrintDevice == PrintsTo.driverStationAndRoboRIO)
            PrintsToRIO(appendedErrorMessage);
    }
    ...
    public void printError(String errorMessage, PrintsTo PrintsTo)
    {
        ...
        // if the error must print to the errorlog on the rio.
        if (PrintsTo == ErrorMessage.PrintsTo.roboRIO || PrintsTo == ErrorMessage.PrintsTo.driverStationAndRoboRIO)
     ...
     */
    public void printError(String errorMessage, PrintsTo PrintsTo, boolean attatchTimeStamp)
    {
        String appendedErrorMessage;
        rioTime = getDate();
        // TODO fix this class to take as a constructor arg
    ...
    private void PrintsToRIO(String errorMessage)
    {
        ...
        try
            {
            final PrintWriter log = new PrintWriter(new BufferedWriter(new FileWriter(errorlogLocation, true)));
     ...
     * Deletes all newlines in the original error message, for formatting and to be
     * safe.
     *
