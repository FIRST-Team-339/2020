 ...
 *
 */
public class OldVisionProcessor extends AutoGenVision {
    ...
    public class ParticleReport implements Comparator<ParticleReport>, Comparable<ParticleReport> {
         ...
         *
         * @param r1 The first particle report in the comparison
         * @param r2 The second particle report in the comparison
         ...
         * particle report
         *
         * @param r The particle report whose area is being compared to <b>this</b>
         *          particle report's
         *
     ...
     *
     * @param ip     the IP of the the axis camera (usually 10.3.39.11)
     * @param camera the brand / model of the camera
     */
    public OldVisionProcessor(String ip, CameraModel camera) {
        // Adds the camera to the cscore CameraServer, in order to grab the
        // stream.
        this.camera = CameraServer.getInstance().addAxisCamera("axis-camera", ip);
     ...
     * @param ip             the IP of the the axis camera (usually 10.3.39.11)
     * @param camera         the brand / model of the camera
     * @param ringlightRelay camera ringlight to pick up retro-reflective tape: this
     *                       is the janky fix
     *
     */
     ...
     * @param ip             the IP of the the axis camera (usually 10.3.39.11)
     * @param camera         the brand / model of the camera
     * @param ringlightRelay - actual relay not just DigitalOutput camera ringlight
     *                       to pick up retro-reflective tape: this is the janky fix
     *
     */

    public OldVisionProcessor(String ip, CameraModel camera, Relay ringlightRelay) {
        // Adds the camera to the cscore CameraServer, in order to grab the
        // stream.
        this.camera = CameraServer.getInstance().addAxisCamera("axis-camera", ip);
     ...
     * Creates the object and starts the camera server
     *
     * @param usbPort The USB camera port number. '0' for default.
     * @param camera  the brand / model of the camera
     */
    public OldVisionProcessor(int usbPort, CameraModel camera) {
        // Adds the camera to the cscore CameraServer, in order to grab the
        // stream.
        this.camera = CameraServer.getInstance().startAutomaticCapture("axis-camera", usbPort);
     ...
     * @param usbPort        The USB camera port number. '0' for default.
     * @param camera         the brand / model of the camera
     * @param ringlightRelay camera ringlight to pick up retro-reflective tape
     */
    ...
    public void processImage() {
        ...
        long errorCode = CameraServer.getInstance().getVideo("axis-camera").grabFrame(image);
        ...
        if (errorCode == 0) {
            System.out.println("There was an error grabbing the image. See below:");
            System.out.println(CameraServer.getInstance().getVideo().getError());
        } // end if
     ...
     *
     * @param exposure     How much light will hit the sensor, in percentage.
     * @param whiteBalence The white balence of the camera. Constants are found in
     *                     the VideoCamera class
     * @param brightness   How bright the image is in post processing, in
     *                     percentage.
     */
    public void setCameraSettings(int exposure, int whiteBalence, int brightness) {
     ...
     * defined above.
     *
     * @param type What kind of image will be saved. If it is ImageType.RAW, then an
     *             image will be saved directly from the camera. if
     *             ImageType.PROCESSED is chosen, then the robot will save the image
    ...
    public void saveImage(ImageType type) {
        ...
        try {
            ...
            Runtime.getRuntime().exec("mkdir -p " + SAVE_IMAGE_PATH + timeStamp);
            // "echo $(ls -1rtd images*| head -n$(($(ls -1d images*| wc -l)-" +
            // numImageFolders+"))) \\> temp.txt"
            // ls -l | find -name "images*" | tail -n +6 | xargs rm -r

            Runtime.getRuntime().exec("ls -ld " + IMAGE_BASIC_PATH + "; sleep 30");
        ...
        // until we get one
        CameraServer.getInstance().getVideo("axis-camera").grabFrame(tempImage);
        ...
        default:
            ...
            // chosen.
            System.out.println("Failed to save image: Image type not recognized.");
            break;
        } // switch
          // Save the image to the folder specified with the name specified

        Imgcodecs.imwrite(SAVE_IMAGE_PATH + timeStamp + "/" + fileName, tempImage);
     ...
     *
     * @param button Whether or not the button is pressed
     * @param type   What kind of image should be saved to the RoboRIO's storage.
     */
     ...
     *
     * @param button 1 (or multiple) joystick button values; recommended to use 2
    ...
    public void takeLitPicture(boolean button) {
        ...
        if (button == true && pictureTakenByButton == false && takePictureByButton == false) {
     ...
     *
     * @param ringLightValue use kForward or kReverse to set the ring light
     */
     ...
     * Set the ring light to a value
     *
     * @param ringLightValue use true to turn the relay on
     */
     ...
     * Turns on the ring light
     *
     * @param button 1 (or multiple) joystick buttons
     */
     ...
     *
     * @param n The index of the size requested. 0 is the largest, and gradually
     ...
     * work with.
     *
     * @param contours The input from the base OpenCV contours output
     */
    private void createParticleReports(List<MatOfPoint> contours) {
        ...
        for (int i = 0; i < reports.length; i++) {
            ...
            reports[i].center = new Point(r.x + (r.width / 2.0), r.y + (r.height / 2.0));
    ...
    public double getPitchAngleDegrees(ParticleReport target) {
        int distFromCenterLine = (int) Math.abs((image.size().height / 2.0) - target.center.y);
        ...
        double focalLengthPixels = image.size().height / (2.0 * Math.tan(verticalFieldOfView / 2.0));
    ...
    public double getYawAngleDegrees(ParticleReport target) {
        int distFromCenterLine = (int) Math.abs((image.size().width / 2.0) - target.center.x);
    ...
    private final static String timeStamp = new SimpleDateFormat("MMddHHmm").format(new Date());
