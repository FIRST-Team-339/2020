...
public class UltraSonic extends AnalogInput
    ...
    // static declarations
    // ---------------------------------------------
    private final double DISTANCE_TRAVELED_IN_SEC = ((8.0 /* ft */ * 12.0) + 6.0);
    ...
    // -------------------------------------------------------
    /**
     * Create an instance of an UltraSonic class. Creates a analog channel given a
     * channel and uses the default slot (module).
     *
     ...
     *                          - the port for the digital input
     * @param scalingFactor
     *                          - how to scale the Analog Channel to keep the
     *                          correct range on the Ultrasonic range finder
     * @author Bob Brown
     * @written Jan 14, 2012 -------------------------------------------------------
     */
     ...
     */
    /*
     * public UltraSonic ( final int slot, final int channel) { super(slot,
     * channel); } // end UltraSonic()
     */
    // -------------------------------------------------------
    /**
     * Create an instance of an UltraSonic class.. Creates a analog channel given a
    ...
    /*
     * public UltraSonic ( final int slot, final int channel, double scalingFactor)
     * { super(slotchannel); this.setScalingFactor(scalingFactor); } // end
     * UltraSonic()
     */
    // -------------------------------------------------------
    /**
     * This function returns the amount of time to keep past sonar values for to be
     * used for past calculations
     *
     * @return double - amount of time to keep past values for - usually about 1
     *         sec. worth
    ...
    // -------------------------------------------------------
    /**
     * This function returns the whether or not we are using Confidence processing
     * in our calculations for this sensor.
     *
     * @return boolean - true - if we are using Confidence false - if we are not
    ...
    // -------------------------------------------------------
    /**
     * This function returns the distance from Kilroy that the nearest object is.
     *
    ...
    public int getDistanceFromNearestBumper()
    {
        ...
        if (normalizedDistance > this.distanceFromNearestBumper)
            {
            normalizedDistance = normalizedDistance - this.distanceFromNearestBumper;
            }
        // -------------------------------------
    ...
    // -------------------------------------------------------
    /**
     * This function returns the distance value using the confidence array. This
     * function is using weighted proximity. If the latest value is close to the
     * last 2 values, then this is considered good. If the latest value is < 10 ft,
     * then it is considered better than a value that is > 10 ft.
     *
    ...
    private int getDistanceValueWithConfidence()
    {
        ...
        double distanceTraveledPerTic = (this.DISTANCE_TRAVELED_IN_SEC / this.sensorPeriodicity) * 2;
        ...
        if ((this.confidenceValueArray[this.presentConfidenceValueIndex] == java.lang.Integer.MAX_VALUE)
            ...
            {
            this.lastGoodValue = this.confidenceValueArray[this.presentConfidenceValueIndex - 1];
            // -------------------------------------
            ...
            // -------------------------------------
            for (index = 1; (index <= (numberToCheck + 1)) && (outlier == false); index++)
                {
                // -------------------------------------
                ...
                if (workConfidenceValueIndex == 0)
                    {
                    workConfidenceValueIndex = this.confidenceValueArray.length - 1;
                    // -------------------------------------
                ...
                if (index == 1)
                    ...
                    if (this.confidenceValueArray[workConfidenceValueIndex] < this.weightedDistance)
                        {
                        distanceTraveledPerTic = distanceTraveledPerTic * this.WEIGHT_FACTOR;
                        }
                    } // if
                // -------------------------------------
                ...
                // -------------------------------------
                if (workConfidenceValueIndex != 0)
                    if (Math.abs(this.confidenceValueArray[workConfidenceValueIndex]
                ...
                // an outlier
                // -------------------------------------
                if ((workConfidenceValueIndex == 0) && (Math.abs(this.confidenceValueArray[workConfidenceValueIndex]
    ...
    // -------------------------------------------------------
    /**
     * This function returns the number of items that we are going to search
     * backwards in the value array to make sure that everything is good.
     *
     * @return int - number of items to check backwards in the confidence array to
     *         make sure that the latest sonar value is "good"
    ...
    // -------------------------------------------------------
    /**
     * gets the distance from the nearest bumper that the Ultrasonic board.
     *
     * @return distance value held by this class that represents the distance from
     *         the nearest bumper to the Ultrasonic board
    ...
    // -------------------------------------------------------
    /**
     * This function returns weather or not we will print the print distance value
     * to drivers station
     *
    ...
    // -------------------------------------------------------
    /**
     * This function returns the distance from the ultrasonic board that the nearest
     * object is.
     *
    ...
    /**
     * This function returns the scaling factor that the Ultrasonic is presently
     * using. The scaling factor is used to multiple the Analog Channel output by.
     * The multiplication factor keeps the range of the Ultrasonic in the correct
     * range. The default scaling factor is 0.5
     *
    ...
    // -------------------------------------------------------
    /**
     * This function returns the sensor Periodicity. It is usually about 50 ms.
     * unless changed by a call setSensorPeriodicity()
     *
    ...
    /**
     * This function returns the weighted distance where withing we give more weight
     * to the values input from the sonar
     *
    ...
    /**
     * This function returns whether or not we are outside a distance close to
     * Kilroy or not.
     *
     * @param rangeValue
     *                       - value to be checked to see if we are outside of this
     *                       value
     * @return boolean - true - if we are farther than the specified value to Kilroy
    ...
    /**
     * This function returns whether or not we are within a distance close to Kilroy
     * or not.
     *
     * @param rangeValue
     *                       - value to be checked to see if we are inside of this
     *                       value
     * @return boolean - true - if we are closer than the specified value to Kilroy
     *         false - if we are not
    ...
    /**
     * This function resets the confidence array so that it is initialized. This
     * would normally be done at the beginning of the run and any time that the
     * sensor is moved.
     *
    ...
    /**
     * This function sets and returns the amount of time to keep past sonar values
     * for to be used for past calculations
     *
     * @param newValue
    ...
    /**
     * This function sets the confidence value for this sensor and returns the
     * whether or not we are using Confidence processing in our calculations for
     * this sensor.
     *
     * @param on
    ...
    /**
     * This function adds the value being place in and then updates the index into
     * the array. Then it calculates whether or not the index is now out of range
     * and if so, resets it.
     *
     * @param newValue
    ...
    private int setDistanceValueIntoConfidenceArray(int newValue)
    {
        ...
        this.presentConfidenceValueIndex = this.presentConfidenceValueIndex % this.confidenceValueArray.length;
    ...
    /**
     * This function sets and returns the number of items that we are going to
     * search backwards in the value array to make sure that everything is good.
     *
     * @param newValue
     *                     - new number of items to check when we get a new sonar
     *                     value
     * @return int - number of items to check backwards in the confidence array to
     *         make sure that the latest sonar value is "good"
    ...
    /**
     * sets and gets the distance from the nearest bumper that the Ultrasonic board.
     *
     * @param newDistance
    ...
    /**
     * This function sets and returns decision on weather or not we want to print
     * distance value to drivers station
     *
     * @param newValue
     *                     - new number of items to check when we get a new true or
     *                     false value
     ...
     * presently using. The scaling factor is used to multiple the Analog Channel
     * output by. The multiplication factor keeps the range of the Ultrasonic in the
     * correct range. The default scaling factor is 0.5
     *
     * @param scalingFactor
    ...
    // -------------------------------------------------------
    /**
     * This function sets and returns the sensor Periodicity. It is usually about 50
     * ms. unless changed by a call setSensorPeriodicity()
     *
     * @param newValue
    ...
    // -------------------------------------------------------
    /**
     * This function sets and returns the weighted distance where withing we give
     * more weight to the values input from the sonar
     *
     * @param newValue
